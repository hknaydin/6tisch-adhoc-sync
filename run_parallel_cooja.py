...
def main():
    ...
    for i in range(args.runs):
        ...
        print(f"[{i+1}/{args.runs}] Starting... Seed: {seed}")
    ...
    for p, log_f, new_csc_path, log_file_path, seed in processes:
        ...
        actual_log_content = ""
        found_testlog = False
        testlog_path = log_file_path # Default if not found
        
        cooja_dir = os.path.expanduser("~/Downloads/Umay/6tisch-ng-sra-v17/6tisch-ng-sra/tools/cooja")
        
        # Scan for the actual testlog file
        for file in os.listdir(cooja_dir):
            if file.startswith("COOJA") and file.endswith(".testlog"):
                candidate_path = os.path.join(cooja_dir, file)
                try:
                    with open(candidate_path, "r", encoding="utf-8") as f:
                        content = f.read()
                        if f"Random seed: {seed}" in content or f"Simulation random seed: {seed}" in content:
                            actual_log_content = content
                            found_testlog = True
                            testlog_path = candidate_path
                            break
                except Exception as e:
                    pass
                    
        if found_testlog:
            res = parse_results(actual_log_content)
        else:
            # Yedek olarak konsol çıktısına da bakalım
            with open(log_file_path, "r", encoding="utf-8") as f:
                actual_log_content = f.read()
            res = parse_results(actual_log_content)

        res["seed"] = seed
        res["log"] = testlog_path
